...
- **producer 目前是 nohup 常驻进程**，未做 systemd 托管；Flink/Kafka 由 compose 管，
  `docker compose -f deploy/kafka-flink/docker-compose.yml up -d` 拉起。宿主重启后需
  手动再起 producer（可在 README/部署文档补 systemd unit）。
