...
**干净重建 topic**（删旧建新）→ **记录并重置 producer 水位**到 `ods_cdc_log` 当前
`max(id)`（记为**重置点 RESET_ID**；只转发重建后的新事件，不重放历史）→ 起 Flink →
重启 producer 并**等 producer 稳定**（已进入轮询循环、水位 >= 重置点、无 batch error）
→ 重提作业 → 端到端自检（默认动态构造一个"必越线且必改变"的余额：取该贷款估值 V，
设余额=0.95×V，LTV≈0.95>0.85 必触发；若与当前余额相同再退到 0.90×V，保证 UPDATE
产生 binlog 事件），断言 **60s 内** inbox 出现 **`event_id` 大于重置点**（且 `loan_id`
匹配）的新预警。自检通过后**自动把测试余额恢复原值**，不把测试值留在业务表里。

> **自检时序竞态的说明**（2026-08-05 加固）：自检 UPDATE 产生的新事件 `id` 一定大于
> 重建前的重置点；producer 以 `id > 水位` 增量转发。若自检发生在 producer 刚重启、
> 尚未进入轮询循环的窗口，事件可能跳过/迟到，自检误报失败。脚本因此：① 重启 producer
> 后先等它稳定（`journal` 出现 `loop start` 且无 `batch error`、水位 >= 重置点）再触发
> 自检；② 断言改用"`event_id` > 重置点"而不是 inbox 基线（避免与旧行混淆，也与
> 重建过程产生的其它新事件天然区分）；③ 等待窗口从 30s 放宽到 60s，覆盖 producer
> 2s 轮询 + Flink 首次消费抖动。
...
- 自检用 `UPDATE` 改一笔贷款余额（默认 30001，`TEST_LOAN_ID` 可覆盖），会产生一条
  新的合法预警行；自检结束后脚本**自动把余额恢复原值**（恢复本身会再产生一条合法
  事件，由正常链路消费，无需等待）。
