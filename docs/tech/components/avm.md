...
> **⚠️ 模型版本声明（避免口径漂移）**：本组件存在多个 `version`，验收与排障务必分清——
> - **canonical r11**（`2026-08-05-r11`）：AC-07 唯一验收模型。canonical 超参
>   `HistGBR + quantile(0.45) + 小区编码 EB 收缩`（28 特征），**45% 覆盖 MAPE 9.88% ≤ 10%**
>   （MdAPE 6.885%，n=3,618）。风险引擎批次与所有验收记录均以此为准。
> - **r20**（`2026-08-06-r20`）：2026-08-06 演示 D4 事件（广州挂牌价下探）驱动的重训快照，
>   **仅用于演示事件传导**（r17→r20），**非 AC-07 验收模型**。历史上它曾覆盖 canonical 产物
>   导致 45% 覆盖 MAPE 退化为 10.324%（>10%），已在 AC-07 验收记录中回退为 r11 口径。
> - **实时/流式路径**：`ads_stream_ltv_alerts` 复用同一 canonical 产物（`output/avm/model.joblib`），
>   其 `model_version` 取决于当前落盘 artifact（默认即 r11）；"实时"指增量计算链路，
>   **不是独立模型版本**——请勿臆造 "实时 r1" 之类的版本号。
