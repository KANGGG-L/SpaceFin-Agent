"""预算表解析（契约 §3.5）：默认值、总额 600/400、IP_BUDGET_JSON 覆盖、非法输入不抛异常。"""
...
def test_default_table_values_and_totals(load_master):
    ...
    t = m.IP_BUDGET
    assert len(t) == 21
    assert t["gz"] == {"sale": 91, "fangyuan": 67}
    assert t["sz"] == {"sale": 91, "fangyuan": 67}
    for c in ("zh", "st", "yf"):
        assert t[c] == {"sale": 22, "fangyuan": 14}
    assert sum(v["sale"] for v in t.values()) == 600
    assert sum(v["fangyuan"] for v in t.values()) == 400
...
def test_ip_budget_json_override_applies(load_master):
    ...
    assert m.IP_BUDGET["st"] == {"sale": 1, "fangyuan": 14}  # 只覆盖给出的字段
    assert m.IP_BUDGET["sz"] == {"sale": 91, "fangyuan": 67}  # 未提及的城保持默认
...
def test_ip_budget_json_invalid_is_ignored_not_raised(load_master):
    for bad in ("{not json", "[1,2,3]", '{"gz": 5}', '{"nocity": {"sale": 1}}', "   "):
        ...
        assert m.IP_BUDGET["gz"] == {"sale": 91, "fangyuan": 67}
        assert sum(v["sale"] for v in m.IP_BUDGET.values()) == 600
